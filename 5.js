...
const getAllCategories = (category, count = 0, categoryResults = []) => {
  if (count === 0)
    count = Math.max(...category.map((item) => item.parentCategoryIds.length));
    count--;

    if (count >= 0 && categoryResults.length === 0) {
        return getAllCategories(
        category,
        count,
        category
            .filter((item) => item.parentCategoryIds.length === count)
            .map((cat) => ({
                id: cat.id,
                displayName: cat.displayName,
                subCategories: [],
                parentCategoryIds: cat.parentCategoryIds,
            }))
        );
    }

    const nextLevel = category.filter(
        (item) => item.parentCategoryIds.length === count
    );

    const finalResults = nextLevel.map((cat) => ({
        id: cat.id,
        parentCategoryIds: cat.parentCategoryIds,
        displayName: cat.displayName,
        subCategories: categoryResults.filter((res) =>
        res.parentCategoryIds.includes(cat.id)
    )}));

    if (count > 0 && categoryResults.length > 0) {
        return getAllCategories(category, count, finalResults);
    }

    return finalResults;
};

console.log(util.inspect(getAllCategories(input), false, null, true));
// console.log(getAllCategories(input));
