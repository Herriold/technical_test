...
const multipleNumber = (array) => {
    const sortASC = array.sort((a, b) => a.reduce((acc, obj) => acc + obj) - b.reduce((acc, obj) => acc + obj));
    return Math.max(...sortASC[1]) * Math.max(...sortASC[sortASC.length - 2]);
}

const result = multipleNumber(input);
console.log(result);
